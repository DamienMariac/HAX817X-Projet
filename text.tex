...
\]
\\
On remarque que, pour tout $t<t^*$ où $t^*$ est la borne supérieure du support des $X_i$, on a $F(t)<1$, 
\[
...
\]
L'idée derrière la théorie des valeurs extremes est d'introduire deux suites ($b_n$) et ($a_n$) (avec $a_n > $  0 pour tout $n$) afin d'avoir une limite non dégénérée pour le maximum d'un échatillon en posant $\frac{M_n - b_n}{a_n}$. 
\\
...
\]
\end{definition}
\noindent On note alors $Y_n = \frac{M_n -b_n}{a_n}$ et sont ésperance devient : 
\[
...
En reprenant les hypothèse de base, on remarque que pour tout $t\in\mathbb R$,
\[
...
les grands sinistres incendie survenus au Danemark entre 1980 et 1990 (Données issue du package \textbf{ismev} de R).
\\
\\

...
\]
Par croissance comparé, comme \(x^{-1/\gamma}\) tend vers 0 relativement lentement, la décroissance de \(S(x)\) est \og moins rapide \fg{} que \(\exp(-\alpha x)\). On a alors : 
\[
S(x)\,\sim\, K\,x^{-1/\gamma} 
\quad (\text{pour } x\to\infty),
\]
ce qui caractérise une \emph{queue lourde} : la probabilité d'observer des valeurs très grandes est plus élevée que dans un modèle à décroissance exponentielle.
...
\\
\\
Autrement dit, il n'y a aucune probabilité d'observer une valeur au-delà de \(x_{\max}\).
\\
\\
...
\]
On retrouve ici une queue à décroissance exponentielle, ce qui est caractéristique d'une \og queue légère \fg{} : la probabilité d'observer des valeurs extrêmes est faible.
\\
\subsection{Résumé}

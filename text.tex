...
\noindent La théorie des valeurs extrêmes s’intéresse donc à des grandeurs 
comme le maximum d’un échantillon $(X_1, X_2, \dots, X_n)$~:
\[
...
\[
x = Q(1-\frac{1}{y}) = U(y) \; \; \; \; \; \; \text{avec $Q$ la fonction quantile}
\]
...
\noindent En reprenant l'expression ci-dessus et en passant au logarithme, on a :
...
Avec le parametre $b_n$ définie au dessus et en posant $u=\frac{1}{v}$ on obtient alors une condition, il faut qu'il existe une fonction $a$ tel que $\lim_{x \to \infty} \frac{U(xu) - U(x)}{a(x)}$ converge \textbf{vers une fonction $h(u)$}.
\\
\\
\begin{proposition}
...
\begin{itemize}
	...
	\item \textbf{Etape 2} : on calcule ensuite la valeur de la fonction en ces 3 points : f est la fonction GEV généralisée et on les trie par valeurs décroissantes.
...
\noindent Un exemple de code pour les simulations de maxima par blocs : 
...
\begin{lstlisting}
	...
	labs(title = "Méthode des maximas par blocs pour une loi uniforme",
	x = "Maxima par blocs",
	y = "Densité") +
	theme_minimal() +
...
\noindent Idem pour les dépassements de seuil : 
...
\begin{lstlisting}
	...
	scale_fill_manual("", values = c("Simulation" = "skyblue")) +
	scale_color_manual("", values = c("Densité théorique" = "red")) +
	labs(title = "Méthode du dépassement de seuil pour une loi uniforme",

...
\noindent On peut alors déterminer la loi limite des maxima, en vertu du théorème de Fisher-Tippett c'est une distribution GEV classique de la forme :
\[
G_{\mu,\sigma,\gamma}(x)
\;=\;
\exp\!\Bigl\{-\bigl[1 + \gamma\,u\bigr]^{-1/\gamma}\Bigr\}.
...
\] \\
\noindent A partir de $\beta_r$, on peut ensuite définir des estimateurs pour $\gamma, a_n$ et $b_n$ qui possèdent de bonnes propriétés (normalité asymptotique) sous certaines conditions. \\

\noindent Sous certaines conditions, on peut enfin démontrer que les quantiles élevés sont facilement estimables par cette méthode. \\
...
\subsection{Méthode des maxima en bloc}
\subsubsection{Principe}
...
\subsubsection{quantile de retour}

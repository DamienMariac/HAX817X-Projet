...
\]
\end{definition}
\noindent Dans notre contexte, on décide de poser $Y_n = \frac{M_n -b_n}{a_n}$ et son espérance devient : 
\[
...
\]
On va alors procéder à un changement de variable. Mais avant on introduit la fonction quantile qui va nous être utile par la suite. Elle est définie par :
\\
\[
Q(p) \;=\; F^{-1}(p)
\;=\;
...
\noindent On pose alors le changement de variable issu de \textit{Statistics of Extremes, Jan Beirlant 2004}: 
...
\]
Et par un développement limité du logarithme :
\[
...
\vspace{0.3cm}
\noindent Il reste à étudier les limites possibles, pour cela nous utilisons la proposition suivante (\textit{Statistics of Extremes, Jan Beirlant 2004}) :
\vspace{0.3cm}
...
\noindent
\begin{remark}
	La constante $c$ ne peut pas être nulle, car cela conduirait à une limite dégénérée pour la variable normalisée $\frac{M_n - b_n}{a_n}$. En pratique, on peut toujours se ramener le cas $c > 0$ au cas $c = 1$ en absorbant $c$ dans la définition de la fonction $a$.
\end{remark}
...
\noindent Cette équation admet comme solution à une constante multiplicative près (admis) :
\[
...
\\
\subsection{Résumé}
\noindent On observe alors qu’apparaît un \textbf{modèle paramétrique} : la loi limite des maxima s’exprime selon une famille de lois qu’on appelle \emph{GEV} (Generalized Extreme Value), paramétrée par trois paramètres :
...
\noindent Ils proviennent des suites de normalisation $a_n > 0$ et $b_n$ introduites dans l’étude de la variable normalisée \(\frac{M_n - b_n}{a_n}\). 

\noindent On pose alors $\mu = b_n \; \text{et} \; \sigma = a_n$, ce qui permet de réécrire la variable réduite \(u = \frac{x - \mu}{\sigma}\). Ainsi, la forme générale de la fonction de répartition limite devient :
\[
G_{\mu, \sigma, \gamma}(x) = \exp\left\{-\left[1 + \gamma \left(\frac{x - \mu}{\sigma} \right)\right]^{-1/\gamma} \right\}.
\]

\noindent Ce modèle permet donc d’ajuster statistiquement la loi des maxima à partir des données, en estimant directement ces trois paramètres. Les lois limites portent un nom différent selon la valeur du paramètre \(\gamma\).
...
\noindent Commençons par étudier une loi uniforme sur [0,1], on peut démontrer que le maximum suit une loi de Weibull de paramètres 1 et 1. \\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{./images/Max_Uniforme.png}
	\caption{Comparaison entre le maximum de 1000 lois uniformes sur [0,1] et la loi Weibull(1,1)}
\end{figure}

\noindent Remarquons que l'on obtient une loi de Weibull, ce qui est logique au vu du fait que ce soit une loi à queue bornée car son support est fini.

\begin{remark}
	Dans les graphes de cette section, la mention Y\_n en légende correspond au maximum normalisé : $ Y_n = \frac{M_n - b_n}{a_n} $.
\end{remark}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{./images/Max_Expo.png}
	\caption{Comparaison entre le maximum de 1000 lois exponentielles de paramètre 1 et la loi  Gumbel(0,1)}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Comparaison entre le maximum de 1000 lois normales centrées réduites et la loi  Gumbel(0,1)}
\end{figure}
...
\noindent Une nouvelle simulation donne :

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{./images/Max_Cauchy.png}
	\caption{Comparaison entre le maximum de 1000 lois de Cauchy(0,1) et la loi Fréchet(1)}
\end{figure}

\noindent Les valeurs extrêmes de la loi de Cauchy décroissent lentement, ce qui est caractéristique d'une loi à queue lourde. La simulation confirme donc que la loi limite est bien une loi de Fréchet comme nous l'avions vu théoriquement dans la section précédente.
...
\]
où l’écart-type asymptotique est donné par :
\[
...
\noindent Donc la variable aléatoire \(\frac{U(V_{n-k_n+1,n}) - U(V_{n-2k_n+1,n})}{U(V_{n-2k_n+1,n}) - U(V_{n-4k_n+1,n})}\) a la même loi que :
...
\noindent Pour mieux étudier le comportement de l’estimateur de Pickands dans divers contextes, nous l’appliquons à des données simulées provenant de plusieurs lois : Pareto, exponentielle, uniforme et Cauchy. Ces distributions ont été sélectionnées car elles illustrent les trois principaux domaines d’attraction des lois de valeurs extrêmes.
...
\noindent L'approche des maxima par blocs (en anglais \textit{Blocks Maxima}) consiste à diviser les n observations en N blocs de taille k. Concrètement, la suite $X_1, ..., X_n$ est divisée en N blocs, le premier bloc est $X_1, ..., X_k$, le second $X_{k+1}, ..., X_{2k}$, etc. On obtient ainsi une suite de maxima $M_1, ..., M_n$ définis sur chacun des blocs.\\
\noindent En général, on considère une période temporelle, comme une journée ou bien une année pour refléter le sens des observations. Dans d'autres cas, c'est moins clair, et le choix du découpage devient crucial car il faut trouver une bonne valeur pour ne pas perdre trop de données tout en conservant suffisamment d'information. \\
...
\] \\
\noindent A partir de $\beta_r$, on peut ensuite définir les trois estimateurs PVM (méthode des moments pondérés) suivants pour $\gamma, a_n$ et $b_n$ qui possèdent de bonnes propriétés asymptotiques sous certaines conditions ($\Gamma$ est la fonction gamma d'Euler). \\
...
\noindent Grâce à ces estimateurs, ou des méthodes plus simples comme des calculs de maximum de vraisemblance, on peut facilement déterminer les valeurs de nos paramètres. \\
...
de données la depassant. Dans le cas d'une distribution à queue bornée il suffit de prendre la borne supérieure de la distribution.
\\
Or, dans le cas d'une distribution à queue lourde et exponentielle ce n'est pas possible. On introduit alors la notion de \textbf{quantile de retour} qui est une valeur que l'on dépasse en moyenne une fois tous les T ans (\textit{Séminaire CNAM 2019}).
\\
...
\\
\\
L'objectif sur ces données est de savoir s'il existe et le cas échéant de le calculer, une valeur telle qu'à l'avenir on la dépasse rarement tout en restant raisonnable. C'est exactement ce que fait le quantile de retour.
\\
\\
Concernant les données de pluie, on dispose de $17531$ observations qui prennent des valeurs entre $0$ et $86,6$. On observe un moyenne de $3,476$ et une médiane de $0,5$.
\\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.48]{./images/rainhisto.png}
	\caption{Distribution des données du fichier rain}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.52]{./images/sinistres.png}
	\caption{Distribution des données du fichier danish}
\end{figure}
...
\\
Via le package \textbf{evd}, on obtient via une estimation par maximum de vraisemblance les paramètres suivants : $\mu = 40,8$, $\sigma = 9,73$ et $\gamma = 0,107$.
\\
L'estimation des paramètres est calculée par maximum de vraisemblance.
\\
\\
On suppose alors que $\gamma \geq 0$, donc il n’existe pas de valeur maximale finie : la probabilité d'avoir de très gros maxima décroît mais n'est pas finie, selon un comportement polynomial ou exponentiel ce qui est embêtant dans le cas pratique surtout quand on cherche des seuils rarement atteints.
\\
\\
...
\noindent On applique la même méthode sur les données de sinistres mais avec une approche légèrement différente puisque les données ne sont pas dans le même format. En effet, on ne dispose pas de données journalières mais des sinistres enregistrés sur une période de 10 ans. C'est à dire qu'il existe des jours sans sinistres qui ne sont pas comptabilisés.
...
En revanche, elle utilise moins de données ce qui la rend plus difficile à utiliser en pratique. En effet, on ne garde que les maximums et on perd donc une partie des données qui peuvent etre conséquents en fonction du choix de $k$.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{./images/pickandsrain.png}
	\caption{Valeur de l'estimateur de Pickands pour rain en fonction de k}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{./images/pickandsdansih.png}
	\caption{Valeur de l'estimateur de Pickands pour danish en fonction de k}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.62]{./images/pickandsdansih.png}
	\caption{Valeur de l'estimateur de Hill pour danish en fonction de k}
\end{figure}
...
\\
En revanche le choix du seuil est important car il faut choisir un seuil suffisamment élevé pour ne pas avoir trop peu de données mais pas trop élevé pour ne pas perdre d'information sur les max. Il faut donc faire le compromis entre le biais et la variance.
...
L'estimation de ce paramètre a pris une bonne partie de notre projet. Nous avons utilisé
...
\noindent Ces codes comparent $n = 10000$ simulations de maxima en blocs pour des lois biens connues par rapport à la loi théorique GEV déterminée grâce à une méthode de vraisemblance du package \textbf{evd}.
...
\noindent Ces codes R illustrent l’application de la méthode des dépassements de seuil à des données simulées suivant des lois bien connues, en vue d’ajuster une loi de Pareto généralisée aux excès au-dessus d’un certain seuil (le quantile choisi est de 95\% ici). On a encore utilisé le package \textbf{evd}.
